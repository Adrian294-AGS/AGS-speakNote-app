...
export const copyText = async (req, res) => {
    const { Id } = req.params;
    try {
        const selectResult = await fetchResultTxt(Id);
        if(!selectResult.result_text){
            return res.status(404).json({success: false, message: `Transcription not found, can't copy`});
        }
